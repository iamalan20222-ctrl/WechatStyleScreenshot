...
- Windows 截图选区右上角新增红色图钉按钮，支持独立置顶贴图、拖动、等比例缩放和多贴图
- `Alt + Shift + P` 支持框选后直接钉图；再次截图时临时隐藏已有贴图

- Windows 截图工具栏新增“译”：本机 OCR 提取文字与行坐标，仅将文字批量发送 DeepSeek，本机覆盖绘制译文；结果在选区内显示并可复制、撤销
- AI 接口设置新增独立 DeepSeek 翻译密钥、模型及目标语言配置，密钥仍使用当前用户 DPAPI 加密
